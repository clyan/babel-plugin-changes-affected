...
import { fileStats as ecmaStats } from '../es-stats'
import type { Import } from '../type'
import denodeify from './denodeify'
import type { DeclarationNode, DependencyMap, Options, Resolver } from './types'
...
interface Extends { [source: string]: Set<string> }
    ...
    if (depMap[extend]) {
      ...
      Object.keys(extendDeclarations).forEach((key) => {
        if (
          ...
          return
        currentDeclarations[key] = { ...extendDeclarations[key], source }
      })
    }
    if (remaining[extend]) {
      ...
      if (!remaining[source].size) delete remaining[source]
    }
